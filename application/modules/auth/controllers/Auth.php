...
class Auth extends MX_Controller
{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->tank_auth->is_max_login_attempts_exceeded($login)) {
				if ($data['use_recaptcha'])
					$this->form_validation->set_rules('g-recaptcha_response', 'Confirmation Code', 'trim|xss_clean|required|callback__check_recaptcha');
				else
			...
			$data['school_name']	=	$this->db->get_where('portal_settings' , array('type'=>'school_name'))->row();
			...
			$this->template
					->set_layout('login')
					->build('auth/login',isset($data) ? $data : NULL);
		}
	}
	...
	function register()
	{
		...
		$use_recaptcha  = ($this->config->item('use_recaptcha') == 'TRUE') ? TRUE : FALSE;

		if ($this->tank_auth->is_logged_in()) {									// logged in
			redirect(base_url().'welocme');
		} elseif ($this->tank_auth->is_logged_in(FALSE)) {						// logged in, not activated
			redirect('/auth/send_again/');

		} elseif ($this->config->item('allow_client_registration', 'tank_auth') == 'FALSE') {	// registration is off
			...
			$data['title'] = 'Create Gurdian Account';
	...
	function register_student()
	{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_student_account(
						$use_username ? $this->form_validation->set_value('username') : '',
						$this->form_validation->set_value('email'),
						$this->form_validation->set_value('password'),
						'5',
			...
			$data['captcha_registration'] = $captcha_registration;
			$data['use_recaptcha'] = $use_recaptcha;
			$this->load->view('auth/register_student', $data);
		}
	}
	/**

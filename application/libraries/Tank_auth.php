...
class Tank_auth
{
	...
	function user_role($r = null)
	{
		return $this->ci->db->select('role')->where('id',$r)->get('roles')->row()->role;
	}

	/**
	 * Create new parent Account on the site and return some data about it:
	 * user_id, username, password, email,id_no, new_email_key (if any).
	 *
	 * @param	string
	 * @param	string
	 * @param	string
	 ...
	 */
    	...
    	function create_user($username, $email, $password, $fullname, $role, $phone, $email_activationl)
	{
	...
	function create_parent($email, $password, $firstname, $lastname,$role,$id_no,$nationality , $marital_status,$title,$phone, $cell_number, $home_address, $postal_address, $employer, $occupation, $work_address, $work_tel, $work_fax, $email_activation)
	{
		if ((strlen($email) > 0) AND !$this->ci->users->is_username_available($email)) {
			...
			if (!is_null($res = $this->ci->users->create_parent($data, $profile, $email_activation))) {
				$data['user_id'] = $res['user_id'];
                $id = $res['user_id'];
				$data['password'] = $password;
    ...
    function create_student($email, $password, $firstname, $middle_name, $lastname,$id_no,$dob, $gender, $parent_id, $home_language, $grade_id,$previous_school,$previous_school_tel,$previous_school_address, $email_activation)
	{
		if ((strlen($email) > 0) AND !$this->ci->users->is_username_available($email)) {
			$this->error = array('username' => 'auth_username_in_use');
			...
			$profile  = array(
				...
				'previous_school_tel' => $previous_school_tel,
			...
			if (!is_null($res = $this->ci->users->create_student($data, $profile, $email_activation))) {
				$data['user_id'] = $res['user_id'];
                $id = $res['user_id'];
				$data['password'] = $password;
	...
	/*
	* Creates new admin user on the site and return some data about  it:
	* user_id, username,  password, email, new_email_key (if any).
	*/

	function admin_create_user($username,$email,$password,$fullname,$role,$phone,$address,$email_activation)
	{
		if ((strlen($username) > 0) AND $this->ci->users->is_username_available($username)){
			$this->error  = array('username' => 'auth_username_in_use' );
		} elseif (!$this->ci->users->is_email_available($email)) {
    	...
    	function portal_settings($school_name,$school_email,$base_url){

		$data = array(
			'school_name' => $school_name,
			'school_email' => $school_email,
			'school_website' => $base_url,

			);
			return $this->ci->db->insert('portal_settings',$data);;
	}
	...
	/**
	 * Check if IDno available for registering.
	 * Can be called for instant form validation.
	 *
	 * @param	string
	 * @return	bool
	 */
	function is_idno_available($id_no)
	{
		return (($id_no > 0) AND $this->ci->users->is_idno_available($id_no));
	}
	...
	/**
	 * Delete parent from the site (only when user is logged in)
	 *
	 * @param	string
	 * @return	bool
	 */

	function delete_parent($password)
	{
		$user_id = $this->ci->session->userdata('user_id');
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				$this->ci->users->delete_parent($user_id);
				$this->logout();
				return TRUE;
